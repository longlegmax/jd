...
!(async () => {
  ...
  if (!($.st <= nowTs && nowTs < $.ed)) {
    ...
    $.http.get({url: `https://github.com/gitupdate/updateTeam/raw/master/redrain.json`}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    let hour = (new Date().getUTCHours() + 8) % 24;
    let redIds = await getRedRainIds();
    if (!redIds) redIds = await getRedRainIds('https://github.com/gitupdate/updateTeam/raw/master/redrain.json');
    $.newAcids = [...(redIds || [])];
